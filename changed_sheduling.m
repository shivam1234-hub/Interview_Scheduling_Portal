function changed_sheduling(affected_candidates)
load ("info.mat","candidates",'interviewers','timing');
  ...
  while i<=size(affected_candidates,2)
      time_slot=2;%getTimeSlot(); % get time slot must be used

      while time_slot<=18
          isScheduled = 0;
      curr_candidate=candidates(affected_candidates(i),:);
                   ...
                   if j==1
                       ...
                       while (curr_queue{1}.isEmpty()~=1)
                           ...
                           if strcmp(interviewers.slots{interviewer_row}(time_slot),'1')==1                              
                               ...
                               candidates(affected_candidates(i),:)=curr_candidate;
                               save('info.mat','candidates','-append');
                               curr_queue{1}.pop();
                               timing{curr_interviewer_id,timing.Properties.VariableNames{time_slot+1}}=affected_candidates(i);
